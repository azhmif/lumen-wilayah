...
class ParseInputStream
{
    ...
    private function file_stream($data)
    {
        ...
        if ( $idx === FALSE ) {
            ...
            if ( $bytes !== FALSE ) {
                $file = new UploadedFile( $path, $filename, $filetype, UPLOAD_ERR_OK );
    ...
    private function parameter($string)
    {
        ...
        if ( preg_match('/name=\"([^\"]*)\"[\n|\r]+([^\n\r].*)?\r$/s', $string, $match) ) {
            if (preg_match('/^(.*)\[\]$/i', $match[1], $tmp)) {
                $data[$tmp[1]][] = ($match[2] && $match[2] !== NULL ? $match[2] : '');
            } else {
                $data[$match[1]] = ($match[2] && $match[2] !== NULL ? $match[2] : '');
            }
        }
        return $data;
    }
